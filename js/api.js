...
const API = (() => {
  ...
  async function call(action, data = {}) {
    ...
    const res = await fetch(url, {method:'POST',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify({action,...data})});
    const text = await res.text();
    let out; try { out = JSON.parse(text); } catch(e) { throw new Error('Invalid response from Google Apps Script. Check the Web App deployment.'); }
  ...
  return {
    call,
    getSettings:()=>call('settings'), getConfig:()=>call('config'), registerStudent:data=>call('register',{data}),
    verifyAdmin:pin=>call('verifyAdmin',{pin}), getStats:(pin,filters)=>call('stats',{pin,filters}),
